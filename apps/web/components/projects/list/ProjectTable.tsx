...
// ── Status Badge (Spesifikasi Presisi: 93x18) ───────────────────────────
...
export default function ProjectTable({ projects, sortBy, sortDir, onSort }: Props) {
  ...
  return (
    <div className="w-full bg-white border border-gray-100 rounded-xl overflow-hidden shadow-sm flex flex-col">
        ...
        <table className="w-full border-collapse">
            ...
            <tr className="bg-[#FBFCFD] border-b border-gray-100">
              ...
              <th className="px-4 py-3 text-left text-[11px] text-gray-400 uppercase tracking-wider">
                <div className="flex items-center gap-1">
                  Project Name 
                  <button onClick={() => onSort('project_name')}><ArrowUpDown size={12} className={sortBy === 'project_name' ? 'text-blue-600' : 'text-gray-300'} /></button>
                </div>
              </th>
              ...
              <th className="px-4 py-3 text-right text-[11px] text-gray-400 uppercase tracking-wider">
                <div className="flex items-center gap-1 justify-end">
                  CPI <button onClick={() => onSort('cpi')}><ArrowUpDown size={12} className={sortBy === 'cpi' ? 'text-blue-600' : 'text-gray-300'} /></button>
                </div>
              </th>
              <th className="px-4 py-3 text-right text-[11px]  text-gray-400 uppercase tracking-wider">
                <div className="flex items-center gap-1 justify-end">
                  SPI <button onClick={() => onSort('spi')}><ArrowUpDown size={12} className={sortBy === 'spi' ? 'text-blue-600' : 'text-gray-300'} /></button>
                </div>
              </th>
          ...
          <tbody className="divide-y divide-gray-50">
            {currentProjects.map((project, idx) => (
              <tr key={project.id} onClick={() => router.push(`/projects/${project.id}`)} className="hover:bg-[#F8F9FC] cursor-pointer transition-colors group">
                ...
                <td className={`px-4 py-4 text-[13px] font-bold text-right ${(project.cpi)}`}>{formatKpi(project.cpi)}</td>
                <td className={`px-4 py-4 text-[13px] font-bold text-right ${(project.spi)}`}>{formatKpi(project.spi)}</td>
      ...
      {/* ── Pagination Footer (Sesuai Gambar Screenshot 00.49.13) ──────────────── */}
      <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between bg-white">
        <div className="text-[13px] text-gray-400 font-medium">
          Showing {Math.min(startIndex + 1, projects.length)} to {Math.min(startIndex + itemsPerPage, projects.length)} of {projects.length} entries
        </div>
